...
class crash_binning:
    ...
    def record_crash(self, pydbg, extra=None):
        ...
        exception_module = pydbg.addr_to_module(
            pydbg.dbg.u.Exception.ExceptionRecord.ExceptionAddress)
        ...
        crash.violation_address = pydbg.dbg.u.Exception.ExceptionRecord.ExceptionInformation[
            1]
        crash.violation_thread_id = pydbg.dbg.dwThreadId
        crash.context = pydbg.context
        crash.context_dump = pydbg.dump_context(
            pydbg.context, print_dots=False)
        ...
        for i in range(len(crash.seh_unwind)):
            ...
            crash.seh_unwind[i] = (
                addr, handler, "%s:%08x" % (module, handler))
